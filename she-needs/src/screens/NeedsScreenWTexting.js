...
import { TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_PHONE_NUMBER, RECIPIENT_PHONE_NUMBER } from '../config';
        ...
        if (item.label === 'SOMETHING\nELSE') {
            Alert.prompt(
                ...
                (text) => {
                    ...
                    sendSMS(`User entered: ${text}`);
                }
            );
        } else {
            sendSMS(`User selected: ${item.label}`);
        }
    };
